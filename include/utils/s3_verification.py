...
def verify_s3_artifacts(run_id: str, expected_artifacts: Optional[List[str]] = None) -> Dict[str, any]:
    ...
    try:
        ...
        # Handle different artifact URI formats
        bucket = "mlflow-artifacts"

        if artifact_uri.startswith("s3://"):
            # Direct S3 URI: s3://bucket/path/to/artifacts
            parts = artifact_uri.replace("s3://", "").split("/", 1)
            bucket = parts[0]
            prefix = parts[1] if len(parts) > 1 else ""
        elif artifact_uri.startswith("mlflow-artifacts:/"):
            # MLflow artifacts protocol: mlflow-artifacts:/experiment_id/run_id/artifacts
            # Convert to S3 path: bucket/run_id_prefix/run_id/artifacts
            path = artifact_uri.replace("mlflow-artifacts:/", "")
            # The actual S3 structure is: bucket/run_id[:2]/run_id[2:4]/run_id/artifacts
            prefix = f"{run_id[:2]}/{run_id[2:4]}/{run_id}/artifacts"
            logger.info(f"Converted mlflow-artifacts URI to S3 prefix: {prefix}")
        else:
            results["errors"].append(f"Unknown artifact URI format: {artifact_uri}")
            return results
        ...
        if 'Contents' in response:
            ...
            if s3_objects:
                logger.info(f"Sample artifacts: {', '.join(s3_objects[:5])}...")

        else:
            results["errors"].append(f"No artifacts found in S3 at bucket={bucket}, prefix={prefix}")
...
def log_s3_verification_results(results: Dict[str, any]):
    ...
    if results["success"]:
        ...
        if results["missing_artifacts"]:
            ...
            if results["s3_artifacts"]:
                logger.info("  - Available artifacts:")
                for art in results["s3_artifacts"]:
                    logger.info(f"    - {art}")