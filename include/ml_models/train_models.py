...
import optuna
import sys
# Configure Optuna to log to stdout so Airflow doesn't treat them as errors
optuna.logging.get_logger("optuna").handlers = [logging.StreamHandler(sys.stdout)]
import mlflow
...
class ModelTrainer:
    ...
    def preprocess_features(self, train_df: pd.DataFrame, val_df: pd.DataFrame, 
        ...
        # Encode categorical variables using OrdinalEncoder to handle unseen categories
        from sklearn.preprocessing import OrdinalEncoder
        
        categorical_cols = X_train.select_dtypes(include=['object']).columns
        for col in categorical_cols:
            if col not in self.encoders:
                # Use OrdinalEncoder with handle_unknown to gracefully handle new categories
                # unknown_value=-1 assigns -1 to any categories not seen during training
                self.encoders[col] = OrdinalEncoder(
                    handle_unknown='use_encoded_value',
                    unknown_value=-1
                )
                X_train[col] = self.encoders[col].fit_transform(X_train[[col]]).ravel()
            else:
                X_train[col] = self.encoders[col].transform(X_train[[col]]).ravel()
            
            X_val[col] = self.encoders[col].transform(X_val[[col]]).ravel()
            X_test[col] = self.encoders[col].transform(X_test[[col]]).ravel()
    ...
    def train_all_models(self, train_df: pd.DataFrame, val_df: pd.DataFrame,
        ...
        try:
            ...
            if not prophet_enabled:
                ...
                # # Calculate weights using inverse RMSE (lower RMSE = higher weight)
                # # Use squared inverse to amplify the difference between models
                # xgb_inv_rmse = 1.0 / (xgb_val_rmse ** 2)
                # lgb_inv_rmse = 1.0 / (lgb_val_rmse ** 2)

                # total_inv_rmse = xgb_inv_rmse + lgb_inv_rmse
                # xgb_weight = xgb_inv_rmse / total_inv_rmse
                # lgb_weight = lgb_inv_rmse / total_inv_rmse

                # # Log the weights and validation metrics
                # logger.info(f"Validation RMSE - XGBoost: {xgb_val_rmse:.4f}, LightGBM: {lgb_val_rmse:.4f}")
                # logger.info(f"Ensemble weights - XGBoost: {xgb_weight:.3f}, LightGBM: {lgb_weight:.3f}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    if os.path.exists(combined_report):
                        ...
                        logger.error("Combined report file not found after creation attempt")
                except Exception as report_error:
                    logger.error(f"Failed to create combined HTML report: {report_error}", exc_info=True)
        ...
        for key, title in sections:
            if key in saved_files:
                ...
                try:
                    ...
                    logger.warning(f"Failed to embed image for {title}: {e}")
                    html_content += f'<p>Failed to load image: {title}</p>'
    ...
    def save_artifacts(self):
        ...
        # Create a dedicated directory for MLflow artifacts to avoid logging garbage files
        mlflow_artifacts_dir = '/tmp/mlflow_artifacts'
        if os.path.exists(mlflow_artifacts_dir):
            shutil.rmtree(mlflow_artifacts_dir)
        os.makedirs(mlflow_artifacts_dir, exist_ok=True)

        # Save scalers and encoders
        joblib.dump(self.scalers, os.path.join(mlflow_artifacts_dir, 'scalers.pkl'))
        joblib.dump(self.encoders, os.path.join(mlflow_artifacts_dir, 'encoders.pkl'))
        joblib.dump(self.feature_cols, os.path.join(mlflow_artifacts_dir, 'feature_cols.pkl'))

        # Save individual models in the expected format
        models_dir = os.path.join(mlflow_artifacts_dir, 'models')
        ...
        if 'xgboost' in self.models:
            joblib.dump(self.models['xgboost'], os.path.join(models_dir, 'xgboost/xgboost_model.pkl'))

        if 'lightgbm' in self.models:
            joblib.dump(self.models['lightgbm'], os.path.join(models_dir, 'lightgbm/lightgbm_model.pkl'))

        if 'ensemble' in self.models:
            joblib.dump(self.models['ensemble'], os.path.join(models_dir, 'ensemble/ensemble_model.pkl'))

        # Log only the dedicated artifacts directory (not all of /tmp/)
        self.mlflow_manager.log_artifacts(mlflow_artifacts_dir)
